...
# 2) Build (User-Site → base="/")
Write-Host "Baue Vite (base=/)..." -ForegroundColor Cyan
npm run build
if ($LASTEXITCODE -ne 0) { Fail "Build fehlgeschlagen." }

# 3) dist → Root (ersetzen) + 404
$Dist = Join-Path $Project "dist"

# Alte Build-Artefakte im Root aufräumen (ohne Source-Dateien anzufassen)
...
Write-Host "dist → Root kopiert, 404.html erstellt." -ForegroundColor Cyan
